  ...
  return skeletons.map((skeleton, index) => (
    <Skeleton
      ...
      className={cn(
        ...
        index > 0 && 'bg-white shadow-sm border border-secondary/10'
      )}
    />
  ))
}
...
const Sessions = () => {
  ...
  return (
    <div className="w-full">
      <div className="mb-2 w-full text-xs font-gotham-regular uppercase text-primary">Sessions</div>
      <div
        className={`h-[calc(100vh-345px)] overflow-y-auto font-gotham-book transition-all duration-300 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar]:transition-opacity [&::-webkit-scrollbar]:duration-300 ${isScrolling ? '[&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-secondary [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar]:opacity-0' : '[&::-webkit-scrollbar]:opacity-100'}`}
